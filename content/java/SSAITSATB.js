...
var SSAITSATB = function(
		...
		var _s = this;
		var prototype = SSAITSATB.prototype;
		...
		_s.setupLeftAndRight = function(){
			...
			_s.leftTxt = new SSAITSDisplayObject("div");
			...
			_s.rightTxt = new SSAITSDisplayObject("div");
		...
		_s.updateTime = function(){
			var hasHours = SSAITSUtils.formatTime(_s.duration).length > 5;
			var totalTime = SSAITSUtils.formatTime(_s.duration);
			_s.rightTime = SSAITSUtils.formatTime(_s.duration * _s.pb);
			_s.leftTime = SSAITSUtils.formatTime(_s.duration * _s.pa);
		...
		_s.setupMainScrubber = function(){
			...
			_s.mainScrubber_do = new SSAITSDisplayObject("div");
			...
			_s.mainScrubberBkMiddle_do = new SSAITSDisplayObject("div");	
			...
			//setup progress bar
			_s.mainScrubberDrag_do = new SSAITSDisplayObject("div");
			...
			if(_s.useHEX){
				_s.mainScrubberDragMiddle_do = new SSAITSDisplayObject("div");
				_s.mainScrubberMiddleImage.onload = function(){
					var testCanvas = SSAITSUtils.getCanvasWithModifiedColor(_s.mainScrubberMiddleImage, controller.nBC, true);
			...
			SSAITSTextButton.setPrototype();
			_s.left_do = new SSAITSTextButton(
				'A',
			...
			_s.left_do.addListener(SSAITSTextButton.MOUSE_DOWN, _s.aDown);
			_s.left_do.addListener(SSAITSTextButton.MOUSE_UP, _s.aUp);

			SSAITSTextButton.setPrototype();
			_s.right_do = new SSAITSTextButton(
				'B',
				 _s.buttonTextNormalColor,
				 _s.buttonTextSelectedColor,
				 _s.buttonBackgroundNormalColor,
			...
			_s.right_do.addListener(SSAITSTextButton.MOUSE_DOWN, _s.bDown);
			_s.right_do.addListener(SSAITSTextButton.MOUSE_UP, _s.bUp);
		}

		_s.bDown = function(e){
			_s.scrub = true
			var vc = SSAITSUtils.getViewportMouseCoordinates(e.e);	
			...
			_s.dispatchEvent(SSAITSATB.START_TO_SCRUB);
		}
		...
		_s.bUp = function(e){
			...
			_s.dispatchEvent(SSAITSATB.STOP_TO_SCRUB);
		}

		_s.bMoveHandler = function(e){
			if(e.preventDefault) e.preventDefault();
			var vc = SSAITSUtils.getViewportMouseCoordinates(e);	
	...
	/* set prototype */
	SSAITSATB.setPrototype = function(){
		SSAITSATB.prototype = null;
		SSAITSATB.prototype = new SSAITSTransformDisplayObject("div");
	};

	SSAITSATB.START_TO_SCRUB = "startToScrub";
	SSAITSATB.SCRUB = "scrub";
	SSAITSATB.STOP_TO_SCRUB = "stopToScrub";

	SSAITSATB.prototype = null;
	window.SSAITSATB = SSAITSATB;
}(window));


/* SSAITSTextButton */
(function (window){
var SSAITSTextButton = function(
		label,
		colorN,
		colorS,
		bkColorN,
		...
		var _s = this;
		var prototype = SSAITSTextButton.prototype;
		...
		_s.setupMainContainers = function(){
			...
			_s.dumy_do = new SSAITSDisplayObject("div");
			if(SSAITSUtils.isIE){
		...
		_s.onMouseDown = function(e){
			...
			_s.dispatchEvent(SSAITSTextButton.MOUSE_DOWN, {e:e});
		};

		_s.checkUp = function(e){
			var vc = SSAITSUtils.getViewportMouseCoordinates(e);	
			if(!SSAITSUtils.hitTest(_s.screen, vc.screenX, vc.screenY)){
				_s.setNormalState();	
				if(!_s.isMbl){
			...
			_s.dispatchEvent(SSAITSTextButton.MOUSE_UP);
		}
	...
	/* set prototype */
	SSAITSTextButton.setPrototype = function(){
		SSAITSTextButton.prototype = null;
		SSAITSTextButton.prototype = new SSAITSDisplayObject("div");
	};
	
	SSAITSTextButton.MOUSE_UP = 'mouseUp';
	SSAITSTextButton.MOUSE_DOWN = 'mouseDown';
	
	SSAITSTextButton.prototype = null;
	window.SSAITSTextButton = SSAITSTextButton;
}(window));